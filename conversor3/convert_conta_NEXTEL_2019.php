...
function converteFaturaNEXTELFixo2019(string $arquivoConvertidoTXT) {
	...
	for ($for1 = 0; $for1 < $quantidadeTotalLinhas; $for1++) {
		...
		if($for1 > 3) {
			...
			if(utf8_encode(trim(substr($arquivo[$conta],0,19))) == 'VEJA    O  USO   DETALHADO         DO   VIVO   ') {
			...
			//$formatoDuracaoLigacao1 = "([0-9]{2,4}[m][0-9]{2})[s]"; // 00m30s
			...
			if($servicosContratados > 0) {
				...
				$servicosCadastrados =    array(
												...
												"Escolha-SMART10GB+Ilimitado193/Pós/SMP"		=> "Escolha-SMART10GB+Ilimitado 193/Pós/SMP",
