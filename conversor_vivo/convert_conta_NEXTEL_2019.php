...
function converteFaturaNEXTELFixo2019(string $arquivoConvertidoTXT) {
	...
	// **********************************************************************************************************************************************************************
		...
		$servicosCadastrados = array(
								...
								"PAC  500 MIN FLEX IND"				=> "PAC  500 MIN FLEX IND",
								"PAC500MINFLEXIND"					=> "PAC  500 MIN FLEX IND",
...
//*****[ CREDITOS ]*****
		...
		if($for1 > 10) {
			str_replace("DescriçãoReferênciaValorR$", "", str_replace($listaEspacos, "", utf8_encode(substr($arquivo[$conta2-1], 0, 200))), $flagCapturaDados);
			if($flagCapturaDados > 0) {
				$capturar14 = 1;
			}
			str_replace(array ("NúmerodaConta:", "Subtotal"), "", str_replace($listaEspacos, "", utf8_encode(substr($arquivo[$conta], 0, 200))), $flagEncerraCapturaDados);
		...
		if($capturar14 == 1) {
			...
			$credito1[1] = $numero_origem; // NUMERO
			$credito1[2] = ''; // DATA
			$credito1[3] = ''; // HORA

			$credito1[4] = trim(utf8_encode(substr($arquivo[$conta], 0, 70))); //DESCRIÇÂO
			// $credito1[4] = 'DESCONTO E PROMOCAO'; //DESCRIÇÂO

			$credito1[5] = ''; // NUMERO DESTINO
			$credito1[6] = ''; // OPERADORA
			$credito1[7] = ''; // MINUTOS
			$credito1[8] = ''; // QUANTIDADE
			$credito1[9] = ''; // MEGA
			$credito1[10] = capturaValor2(trim(substr($arquivo[$conta], -13))); // VALOR

			$valor = str_replace(array(".", ","), array("", "."), $credito1[10]);
			$totalCapturado += $valor;

			if(empty($todosdescricaoServicos["{$credito1[4]}"])) {
				$str = trim($credito1[4]);
				$todosdescricaoServicos["{$credito1[4]}"] = 0;
			}
	...
	// ================ [ AGRUPAMENTO DOS DESCONTOS] =================
			...
			if($modoDebug == 1) {
				...
				$str = trim($credito1[4]);
				if(empty($debugcredito1["{$str}"])) {

					$debugcredito1["{$str}"] = 0;
				}

				if(empty($debugcredito11["{$str}"])) {

					$debugcredito1["{$str}"] += $valor;
				}else {
	...
	// ===============================================================
			...
			$linhacredito1 = 
				...
				.$credito1[2].';'
				.$credito1[3].';'
				.iconv("UTF-8", "Windows-1252", $credito1[4]).';'
				.$credito1[5].';'
				.$credito1[6].';'
				.$credito1[7].';'
				.$credito1[8].';'
